...
def parse_output(out_ints):
    ...
    for row in rows:
        ...
        if len(row) == 0: 
            ...
            continue
        if len(read) > 0: exec(read + ".append(row.replace('- ',''))")
    return room_name, room_desc, doors, items

# Move droid from current room to destination room
# Both nodes must already exist in map_graph with a path connecting them
def move_to_node(comp, map_graph, droid_loc, destination, clear_output = True):
    move_path = nx.shortest_path(map_graph, source=droid_loc, target = destination)
    for node in move_path[1:]:
        door = map_graph.get_edge_data(droid_loc, node)['directions'][droid_loc] # Find which door to move to next node
        comp.ascii_command(door) # Order droid to go through door
        if(clear_output): comp.outputs = []
        droid_loc = node
    return droid_loc

def visualize_graph(map_graph, edge_text = False):
    labels = nx.get_edge_attributes(map_graph,'directions')
    short_labels = {}
    for edge in labels:
        label = []
        for r in labels[edge]:
            d = labels[edge][r]
            label.append(r[:2] + ':' + d[0])
        short_labels[edge] = '-'.join(label)
    pos = nx.spring_layout(map_graph)
    nx.draw_networkx_nodes(map_graph,pos)
    nx.draw_networkx_edges(map_graph,pos)
    nx.draw_networkx_labels(map_graph,pos)
    if edge_text: nx.draw_networkx_edge_labels(map_graph,pos,edge_labels=short_labels)
    plt.show()

# Approach:
#   1. Apply Depth-First Search (DFS) algorithm. Get the droid to move room by room to get a complete graph.
#   2. Try picking up each item, see how the droid reacts. Finalize a list of safe & collectable items.
#   3. Take all collectable items, move to SecurityCheckpoint & drop all items there.
#   4. Try each combinations of items until getting the correct weight to pass Pressure-SensitiveFloor.
def part1(input):
    node_dict = {}
    map_graph = nx.Graph()
