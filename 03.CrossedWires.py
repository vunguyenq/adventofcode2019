...
def parse_input(input):
    ...
    for row in input.split('\n'):
        ...
        for move in row.split(','):
            wire.append((move[0],int(move[1:])))
        wires.append(wire)
    return wires

# populate all coordinates of a wire based on its moving directions
def move_wire(wire):
    locs = [(0,0)]
    pos = [0,0]
    for move in wire:
...
# similar to above function, now location with 3rd param to track how many steps passed to reach the point
# now use a dictionatry to track visited points. 2nd visit to the same point will NOT be updated
def move_wire_count_steps(wire):
    locs = {(0,0):0}
    pos = [0,0,0]
    for move in wire:
        direction, step = move
        if(direction == 'U'):
            for i in range(1,step+1):
                #locs.append((pos[0],pos[1]+i,pos[2]+i))
                key = (pos[0],pos[1]+i)
                if key not in locs:
                    locs[key] = pos[2]+i
            pos[1] += step
            ...
            for i in range(1,step+1):
                #locs.append((pos[0]-i,pos[1],pos[2]+i))
                key = (pos[0]-i,pos[1])
                if key not in locs:
                    locs[key] = pos[2]+i
            pos[0] -= step
...
def part1(input):
    wire1, wire2 = input
    wire1_locs = move_wire(wire1)
    wire2_locs = move_wire(wire2)
    intersections = wire1_locs & wire2_locs
    intersections.remove((0,0))
    #print(intersections)
    min_dist = min([abs(p[0]) + abs(p[1]) for p in intersections])
    return min_dist

def part2(input):
    wire1, wire2 = input
    wire1_locs = move_wire_count_steps(wire1)
    wire2_locs = move_wire_count_steps(wire2)
    intersections = set(wire1_locs.keys()) & set(wire2_locs.keys())
    intersections.remove((0,0))
    #print(intersections)
    steps = []
    for p in intersections:
        steps.append(wire1_locs[p] + wire2_locs[p])
    return min(steps)
